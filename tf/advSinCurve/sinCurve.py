...
sin = data_df['sin']
#theta = data_df['theta']
#theta = data_df[['theta', 'thetaSqr']].as_matrix()
theta = data_df[['theta', 'thetaSqr']]
...
model.add(tf.keras.layers.Dense(units = 5, activation = 'sigmoid', input_shape = [2]))
...
#ans = [model.predict([q]) for q in x_test]
ans = model.predict(x_test)

plt.scatter(np.asarray(x_test).transpose()[0], ans, s=area, c=colors, alpha=0.5)
plt.scatter(np.asarray(x_test).transpose()[0], y_test, s=area, c='blue', alpha=0.5)
plt.title('awdwa')
plt.xlabel('x')
plt.ylabel('y')
plt.show()
